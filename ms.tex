...
equations being solved when modelling demography of plants, patches and metapopulations; \ref{sec:FFW16}) a description of the \texttt{FFW16}
physiological model, and \ref{sec:code}) a compendium of worked code examples.
...
% TODO: Table XXX?
% NOTE: I've moved FFW16b to FFdev for now so that it matches the implementation.
...
\ref{sec:code}). Second, we allow for changes in key parameters to
...
% NOTE: is this OK as a merge (was an orphan sentence in previous version).
A variety of algorithms from the area of adaptive dynamics could be applied for modelling community assembly \citep[e.g.][]{Geritz-1998, Falster-2015}, using the estimate of invasion fitness provided by {\plant}.
...
\caption{Physiological dynamics for individual plants varying in size,
  ...
  \textsc{lma} species, due to their longer leaf lifespan. (c)
  ...
  coloured ticks.  See Appendix \ref{sec:code} for further details and
  code.}
\label{fig:plant}
\end{figure}
...
\caption{Population dynamics for two species competing within a patch.
  ...
  metapopulation structure.  See Appendix \ref{sec:code} for further
  details and code.}
\label{fig:patch}
\end{figure}
...
See Appendix \ref{sec:code} for further details and code.}
\label{fig:emergent}
\end{figure}
...
\caption{Example fitness landscape for \textsc{lma} (leaf mass per
  ...
  invasion is possible.  See Appendix \ref{sec:code} for further
  details and code.}
\label{fig:fitness}
\end{figure}
...
A compendium of worked code examples showing how to intearct with {\plant} from R
See attached file \url{vignettes/combined.pdf}.
