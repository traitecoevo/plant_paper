make_fitness_data <- function() {
  ...
  communities <-
    list(first      = 0.10,           # first individual
         ...
         second1    = c(0.08, 0.22),  # ~= the max fitness
         second2    = c(0.08, 0.25),  # ~= the max fitness
         second3    = c(0.08, 0.30),  # ~= the max fitness
         attractor2 = c(0.08, 0.267)) # stable
...
figure_fitness_landscape <- function(data) {
  ...
  label_panel(2)
  mtext(expression("Leaf mass per unit area"~("LMA;"~kg~m^-2)), 1, 3, cex=1)
